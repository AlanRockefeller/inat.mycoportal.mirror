...
def get_all_observations(username):
    ...
    while not finished:
        ...
        try:
            ...
            if "errors" in parsed:
                if "details" in parsed["errors"][0]:
                    if re.match("User .* does not exist", parsed["errors"][0]["details"]):
                        print("\nMO does not seem to recognize your username. Open \"PRIVATE settings.txt\", delete the line for your username, and try again. Make sure it matches your Login at https://mushroomobserver.org/account/prefs.")
                        exit(0)

            print("Response did not include results. Dumping and quitting.\n")
            with open("JSON_dump.json", "w") as outf:
...
def get_single_obs(username, MOID, api_key):
    ...
    answer = careful_request("GET", build_request_URL(base_URL, endpoint, params), headers = USER_AGENT)
...
def read_splist(list_ID):

    base_URL = "https://mushroomobserver.org/api2"
    endpoint = "/observations"
    params = {"species_list" : list_ID, 
                    "detail" : "none",
                    "format" : "json"}
...
def get_notes(MOID):
    ...
    try:
        notes = parsed["results"][0]["notes"]
    except:
        print("Warning: Couldn't get notes from obs "+MOID+".")
        return ""
    else:
        return notes
...
#view_particular("371217")

#full_obses = get_full_obses("myxomop",0,None)
#print("LENGTH IS "+str(len(full_obses)))
#print(json.dumps(full_obses, indent=4, sort_keys=False)[:700])